...
class App(ctk.CTk):
    ...
    def update_bmi(self, *args):
        ...
        self.bmi_str.set(bmi_result)

    def __change_title_bar_color(self):
...
class WeightInput(ctk.CTkFrame):
    def __init__(self, parent, weight_float, metric_bool):
        ...
        btn_minus.grid(row=0, column=0, sticky='ns', padx=8, pady=8)

        btn_small_minus = ctk.CTkButton(
            self,
            text='-',
            font=font,
            text_color=BLACK,
        ...
        btn_small_minus.grid(row=0, column=1, padx=4, pady=4)

        btn_small_plus = ctk.CTkButton(
            self,
            text='+',
            font=font,
            text_color=BLACK,
        ...
        btn_small_plus.grid(row=0, column=3, padx=4, pady=4)
        ...
        btn_plus.grid(row=0, column=4, sticky='ns', padx=8, pady=8)
    ...
    def update_weight(self, info=None):
        ...
        if self.metric_bool.get():
            self.output_str.set(f'{round(self.weight_float.get(), 1)}kg')
        else:
            raw_ounces = self.weight_float.get() * 2.20462 * 16
            pounds, ounces = divmod(raw_ounces, 16)
            self.output_str.set(f'{int(pounds)}lb {int(ounces)}oz')
...
class HeightInput(ctk.CTkFrame):
    ...
    def update_text(self, amount):
        if self.metric_bool.get():
            ...
            meter = text_str[0]
            cm = text_str[1:]
            self.output_str.set(f'{meter}.{cm}m')
        else:
            feet, inches = divmod(amount/2.54, 12)
            self.output_str.set(f'{int(feet)}\'{int(inches)}\"')
